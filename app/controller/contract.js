...
class ContractController extends Controller {
  async summary() {
    ...
    ctx.body = {
      ...
      ...summary.type === 'brc20' ? {
        brc20: {
          name: summary.brc20.name,
          symbol: summary.brc20.symbol,
          decimals: summary.brc20.decimals,
          totalSupply: summary.brc20.totalSupply.toString(),
          version: summary.brc20.version,
          holders: summary.brc20.holders,
          transactions: summary.brc20.transactions
        }
      } : {},
      ...summary.type === 'brc721' ? {
        brc721: {
          name: summary.brc721.name,
          symbol: summary.brc721.symbol,
          totalSupply: summary.brc721.totalSupply.toString()
        }
      } : {},
  ...
  async brc20BalanceHistory() {
    ...
    let {totalCount, transactions} = await ctx.service.brc20.getBRC20BalanceHistory([ctx.state.contract.contractAddress], tokenAddress)
    ctx.body = {
      totalCount,
      transactions: transactions.map(tx => ({
        id: tx.id.toString('hex'),
