...
class TransactionService extends Service {
  async getTransaction(id) {
    const {
      ...
      Contract, Brc20: BRC20, Brc721: BRC721,
      where, col
    } = this.ctx.model
    const {in: $in} = this.app.Sequelize.Op
    const {Address: RawAddress} = this.app.btcbaminfo.lib
        ...
        if (output.evmReceipt) {
          ...
          outputObject.evmReceipt.logs = eventLogs.filter(log => log.receiptId === output.evmReceipt._id).map(log => ({
            ...
            ...log.brc20 ? {
              brc20: {
                name: log.brc20.name,
                symbol: log.brc20.symbol,
                decimals: log.brc20.decimals
              }
            } : {},
            ...log.brc721 ? {
              brc721: {
                name: log.brc721.name,
                symbol: log.brc721.symbol
              }
            } : {}
          }))
        }
  ...
  async getRawTransaction(id) {
    ...
    const {Transaction: RawTransaction, Input, Output, OutputScript} = this.app.btcbaminfo.lib
  ...
  async sendRawTransaction(data) {
    let client = new this.app.btcbaminfo.rpc(this.app.config.btcbaminfo.rpc)
  ...
  async transformTransaction(transaction, {brief = false} = {}) {
    ...
    let [brc20TokenTransfers, brc20TokenUnconfirmedTransfers, brc721TokenTransfers] = await Promise.all([
      this.transformBRC20Transfers(transaction.outputs),
      confirmations === 0 ? this.transformBRC20UnconfirmedTransfers(transaction.outputs) : undefined,
      this.transformBRC721Transfers(transaction.outputs)
    ])
    ...
    return {
      ...
      brc20TokenTransfers,
      brc20TokenUnconfirmedTransfers,
      brc721TokenTransfers
    }
  }

  transformInput(input, index, transaction, {brief}) {
    const {InputScript, OutputScript} = this.app.btcbaminfo.lib
  ...
  transformOutput(output, index, {brief}) {
    const {OutputScript} = this.app.btcbaminfo.lib
  ...
  async transformBRC20Transfers(outputs) {
    const TransferABI = this.app.btcbaminfo.lib.Solidity.brc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
    for (let output of outputs) {
      if (output.evmReceipt) {
        for (let {addressHex, topics, data, brc20} of output.evmReceipt.logs) {
          if (brc20 && topics.length === 3 && Buffer.compare(topics[0], TransferABI.id) === 0 && data.length === 32) {
  ...
  async transformBRC721Transfers(outputs) {
    const TransferABI = this.app.btcbaminfo.lib.Solidity.brc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
    for (let output of outputs) {
      if (output.evmReceipt) {
        for (let {addressHex, topics, brc721} of output.evmReceipt.logs) {
          if (brc721 && topics.length === 4 && Buffer.compare(topics[0], TransferABI.id) === 0) {
            let [from, to] = await this.ctx.service.contract.transformHexAddresses([topics[1].slice(12), topics[2].slice(12)])
            result.push({
              address: addressHex.toString('hex'),
              addressHex: addressHex.toString('hex'),
              name: brc721.name,
              symbol: brc721.symbol,
              ...from && typeof from === 'object' ? {from: from.hex.toString('hex'), fromHex: from.hex.toString('hex')} : {from},
              ...to && typeof to === 'object' ? {to: to.hex.toString('hex'), toHex: to.hex.toString('hex')} : {to},
  ...
  async getContractTransaction(receiptId) {
    const {Address: RawAddress, OutputScript} = this.app.btcbaminfo.lib
    const {
