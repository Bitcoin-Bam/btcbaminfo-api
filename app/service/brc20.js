...
class BRC20Service extends Service {
  async listBRC20Tokens() {
    const db = this.ctx.model
    const {Brc20Statistics: BRC20Statistics} = db
    const {sql} = this.ctx.helper
    const {gt: $gt} = this.app.Sequelize.Op
    let {limit, offset} = this.ctx.state.pagination

    let totalCount = await BRC20Statistics.count({
    ...
    let list = await db.query(sql`
      SELECT
        ...
        brc20.name AS name, brc20.symbol AS symbol, brc20.decimals AS decimals, brc20.total_supply AS totalSupply,
        brc20.version AS version,
        list.holders AS holders,
        list.transactions AS transactions
      FROM (
        SELECT contract_address, holders, transactions FROM brc20_statistics
        WHERE transactions > 0
        ORDER BY transactions DESC
        LIMIT ${offset}, ${limit}
      ) list
      INNER JOIN brc20 USING (contract_address)
  ...
  async getAllBRC20Balances(hexAddresses) {
    if (hexAddresses.length === 0) {
      return []
    }
    const {OutputScript, Solidity} = this.app.btcbaminfo.lib
    const transferABI = Solidity.brc20ABIs.find(abi => abi.name === 'transfer')
    const {
      Address, TransactionOutput,
      Contract, EvmReceipt: EVMReceipt, Brc20: BRC20, Brc20Balance: BRC20Balance,
      where, col
    } = this.ctx.model
    const {in: $in} = this.app.Sequelize.Op
    let list = await BRC20.findAll({
    ...
    let mapping = new Map(list.map(item => [
      item.contract.addressString,
      {
        ...
        symbol: item.symbol,
        decimals: item.decimals,
        balance: item.contract.brc20Balances.map(({balance}) => balance).reduce((x, y) => x + y),
        unconfirmed: {
          received: 0n,
          sent: 0n
        }
      ...
      if (byteCode.length === 68
        ...
        if (mapping.has(item.output.address.contract.addressString)) {
          ...
          data = {
            ...
            name: item.output.address.contract.brc20.name,
            symbol: item.output.address.contract.brc20.symbol,
            decimals: item.output.address.contract.brc20.decimals,
            balance: 0n,
            unconfirmed: {
              received: 0n,
              sent: 0n
  ...
  async getBRC20Balance(rawAddresses, tokenAddress) {
    const {Address: RawAddress, OutputScript, Solidity} = this.app.btcbaminfo.lib
    const transferABI = Solidity.brc20ABIs.find(abi => abi.name === 'transfer')
    const {
      Address, TransactionOutput,
      Contract, EvmReceipt: EVMReceipt, Brc20: BRC20, Brc20Balance: BRC20Balance,
    ...
    let token = await BRC20.findOne({
      where: {contractAddress: tokenAddress},
      attributes: ['name', 'symbol', 'decimals'],
      transaction: this.ctx.state.transaction
    })
    let list = await BRC20Balance.findAll({
  ...
  async getBRC20BalanceHistory(addresses, tokenAddress) {
    const TransferABI = this.app.btcbaminfo.lib.Solidity.brc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    const {
      Header, Transaction,
      EvmReceipt: EVMReceipt, EvmReceiptLog: EVMReceiptLog,
      Contract, Brc20: BRC20, Brc20Balance: BRC20Balance,
      literal
    } = db
    ...
    let [{totalCount}] = await db.query(sql`
      ...
      FROM evm_receipt receipt, evm_receipt_log log, brc20
      WHERE receipt._id = log.receipt_id AND log.address = brc20.contract_address AND ${{raw: logFilter}}
    `, {type: db.QueryTypes.SELECT, transaction: this.ctx.state.transaction})
    if (totalCount === 0) {
      return {totalCount: 0, transactions: []}
    }
    let ids = (await db.query(sql`
      ...
      INNER JOIN (
        SELECT DISTINCT(receipt.transaction_id) AS id FROM evm_receipt receipt, evm_receipt_log log, brc20
        WHERE receipt._id = log.receipt_id AND log.address = brc20.contract_address AND ${{raw: logFilter}}
    ...
    if (list.length > 0) {
      let intialBalanceList = await BRC20Balance.findAll({
        where: {
      ...
      for (let log of logs) {
        ...
        if (item) {
          ...
          result.tokens.push({
            ...
            name: log.brc20.name.toString(),
            symbol: log.brc20.symbol.toString(),
            decimals: log.brc20.decimals,
            amount: delta
          })
        }
      }
  ...
  async getAllBRC20TokenTransactions() {
    const TransferABI = this.app.btcbaminfo.lib.Solidity.brc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    let {limit, offset, reversed = true} = this.ctx.state.pagination
    ...
    let [{totalCount}] = await db.query(sql`
      ...
      FROM brc20, evm_receipt_log log
      WHERE brc20.contract_address = log.address AND log.topic1 = ${TransferABI.id}
    `, {type: db.QueryTypes.SELECT, transaction: this.ctx.state.transaction})
    let transactions = await db.query(sql`
      SELECT
        ...
        brc20.name AS name,
        brc20.symbol AS symbol,
        brc20.decimals AS decimals,
        evm_receipt_log.topic2 AS topic2,
        evm_receipt_log.topic3 AS topic3,
        evm_receipt_log.data AS data
      FROM (
        SELECT log._id AS _id FROM brc20, evm_receipt_log log
        WHERE brc20.contract_address = log.address AND log.topic1 = ${TransferABI.id}
      ...
      INNER JOIN brc20 ON brc20.contract_address = evm_receipt_log.address
  ...
  async getBRC20TokenTransactions(contractAddress) {
    const TransferABI = this.app.btcbaminfo.lib.Solidity.brc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
  ...
  async getBRC20TokenRichList(contractAddress) {
    const db = this.ctx.model
    const {Brc20Balance: BRC20Balance} = db
    const {ne: $ne} = this.app.Sequelize.Op
    let {limit, offset} = this.ctx.state.pagination

    let totalCount = await BRC20Balance.count({
  ...
  async updateBRC20Statistics() {
    const TransferABI = this.app.btcbaminfo.lib.Solidity.brc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {Brc20: BRC20, Brc20Statistics: BRC20Statistics} = db
    const {sql} = this.ctx.helper
    let transaction = await db.transaction()
    try {
      let result = (await BRC20.findAll({attributes: ['contractAddress'], transaction})).map(
        ({contractAddress}) => ({contractAddress, holders: 0, transactions: 0})
      )
      let balanceResults = await db.query(sql`
        SELECT contract_address AS contractAddress, COUNT(*) AS count FROM brc20_balance
      ...
      await db.query(sql`DELETE FROM brc20_statistics`, {transaction})
      await BRC20Statistics.bulkCreate(result, {validate: false, transaction, logging: false})
