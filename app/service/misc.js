...
class MiscService extends Service {
  async classify(id) {
    ...
    const {Block, Transaction, Contract, Brc20: BRC20, where, fn, literal} = db
    ...
    let brc20Results = (await BRC20.findAll({
      where: {
        [$or]: [
      ...
      transaction
    })).map(brc20 => brc20.contractAddress)
    if (brc20Results.length === 0) {
      brc20Results = (await BRC20.findAll({
        where: {
          [$or]: [
    ...
    if (brc20Results.length) {
      let [{addressHex}] = await db.query(sql`
        SELECT contract.address_string AS address, contract.address AS addressHex FROM (
          SELECT contract_address FROM brc20_statistics
          WHERE contract_address IN ${brc20Results}
          ORDER BY transactions DESC LIMIT 1
        ) brc20_balance
        INNER JOIN contract ON contract.address = brc20_balance.contract_address
