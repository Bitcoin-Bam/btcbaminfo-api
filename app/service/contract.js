...
class ContractService extends Service {
  async getContractAddresses(list) {
    const {Address} = this.app.btcbaminfo.lib
  ...
  async getContractSummary(contractAddress, addressIds) {
    const {Contract, Brc20: BRC20, Brc20Statistics: BRC20Statistics, Brc721: BRC721} = this.ctx.model
    const {balance: balanceService, brc20: brc20Service, brc721: brc721Service} = this.ctx.service
    ...
    let [
      ...
      brc20Service.getAllBRC20Balances([contractAddress]),
      brc721Service.getAllBRC721Balances([contractAddress]),
    ...
    return {
      ...
      ...contract.type === 'brc20' ? {
        brc20: {
          name: contract.brc20.name,
          symbol: contract.brc20.symbol,
          decimals: contract.brc20.decimals,
          totalSupply: contract.brc20.totalSupply,
          version: contract.brc20.version,
          holders: contract.brc20.statistics.holders,
          transactions: contract.brc20.statistics.transactions
        }
      } : {},
      ...contract.type === 'brc721' ? {
        brc721: {
          name: contract.brc721.name,
          symbol: contract.brc721.symbol,
          totalSupply: contract.brc721.totalSupply
        }
      } : {},
    ...
    let [{count}] = await db.query(sql`
      SELECT COUNT(*) AS count FROM (
        ...
        WHERE log.receipt_id = receipt._id
          AND ${this.ctx.service.block.getRawBlockFilter('receipt.block_height')}
          AND contract.address = log.address AND contract.type IN ('brc20', 'brc721')
          AND log.topic1 = ${TransferABI.id}
          AND (log.topic2 = ${topic} OR log.topic3 = ${topic})
          AND (
            (contract.type = 'brc20' AND log.topic3 IS NOT NULL AND log.topic4 IS NULL)
            OR (contract.type = 'brc721' AND log.topic4 IS NOT NULL)
          )
      ) list
  ...
  async getContractBasicTransactions(contractAddress) {
    const {Address, OutputScript} = this.app.btcbaminfo.lib
    const {
