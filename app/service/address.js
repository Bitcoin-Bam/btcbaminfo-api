...
class AddressService extends Service {
  async getAddressSummary(addressIds, p2pkhAddressIds, rawAddresses) {
    const {Address} = this.ctx.app.btcbaminfo.lib
    ...
    let [
      ...
      brc20Service.getAllBRC20Balances(hexAddresses),
      brc721Service.getAllBRC721Balances(hexAddresses),
  ...
  async getAddressTransactionCount(addressIds, rawAddresses) {
    const {Address: RawAddress, Solidity} = this.app.btcbaminfo.lib
  ...
  async getAddressBRC20TokenTransactionCount(rawAddresses, token) {
    const {Address, Solidity} = this.app.btcbaminfo.lib
    const TransferABI = Solidity.brc20ABIs.find(abi => abi.name === 'Transfer')
  ...
  async getAddressBRC20TokenTransactions(rawAddresses, token) {
    const {Address, Solidity} = this.app.btcbaminfo.lib
    const TransferABI = Solidity.brc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    ...
    let topicAddresses = rawAddresses
      .filter(address => address.type === Address.PAY_TO_PUBLIC_KEY_HASH)
      .map(address => Buffer.concat([Buffer.alloc(12), address.data]))
    let totalCount = await this.getAddressBRC20TokenTransactionCount(rawAddresses, token)
    let transactions = await db.query(sql`
      SELECT
      ...
      INNER JOIN brc20 ON brc20.contract_address = log.address
  ...
  async getAddressBRC20TokenMempoolTransactions(rawAddresses, token) {
        ...
        },
        {
          ...
          include: [{
            ...
            include: [{
              ...
              as: 'contract',
              required: true,
              where: {address: token.contractAddress, type: 'brc20'},
              attributes: []
            }]
          }]
        },
